...
| [htdt/godogen](https://github.com/htdt/godogen) | **Godogen**: Claude Code skills that build complete Godot 4 projects from game descriptions — architecture planning, GDScript generation, 2D/3D asset creation (Gemini/Tripo3D), visual QA with screenshot analysis; runs on commodity hardware |
...
| [llm-sast-scanner](https://github.com/SunWeb3Sec/llm-sast-scanner) | LLM-oriented SAST skill for structured source-to-sink taint analysis across **34 vulnerability classes**; supports Java/Python/JS-TS/PHP/.NET with a Judge verification step to reduce false positives and actionable remediation output |
